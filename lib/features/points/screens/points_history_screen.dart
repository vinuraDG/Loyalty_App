...
class _PointsHistoryScreenState extends State<PointsHistoryScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final allTxs = svc.getForUser(widget.userId).toList();

    // ── Aggregates ────────────────────────────────────────────────────────
    final totalEarned = allTxs
        .where((tx) => tx.isEarned)
        .fold<int>(0, (s, tx) => s + (tx.points as int));
    ...
    final filtered = allTxs.where((tx) {
      if (_filter == _Filter.earned) return tx.isEarned;
      if (_filter == _Filter.redeemed) return !tx.isEarned;
      return true;
    }).toList();
    ...
    for (final tx in filtered) {
      final label = _dateLabel(tx.date as DateTime);
...
class _HistoryTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final isEarned = tx.isEarned as bool;
    final time = tx.date is DateTime
        ? _timeString(tx.date as DateTime)
        : '';
